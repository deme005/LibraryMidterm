...
namespace Library.Services.Services
{
    public class BorrowService : IBorrowService
    {
        ...
        public void ApproveRequest(string borrowRecordKey)
        {
            ...
            if (book == null || book.Quantity <= 0) throw new InvalidOperationException("Book is out of stock or does not exist.");
        ...
        public void RequestBorrow(ClientUser client, string bookKey)
        {
            ...
            if (!client.IsVerified) throw new UnauthorizedAccessException("Account must be verified to borrow books.");

            var book = _bookRepository.GetByKey(bookKey);
            if (book == null) throw new Exception("Book not found.");
            if (book.Quantity <= 0) throw new InvalidOperationException("Book is out of stock.");
        ...
        public void ReturnBook(string borrowRecordKey)
        {
            var record = _borrowRepository.GetByKey(borrowRecordKey);
            if (record == null) throw new Exception("Borrow record not found.");
            if (record.BorrowStatus != Status.Approved) throw new InvalidOperationException("Only active approved borrows can be returned.");
